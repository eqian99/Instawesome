...
- (void)getEvents:(NSString *)latitude withLongitude:(NSString *)longitude withCompletion:(void(^)(NSDictionary *categories, NSError *error))completion {
    ...
    NSString *url = @"https://api.yelp.com/v3/events?limit=1";
    NSURL *completeURL = [NSURL URLWithString:[NSString stringWithFormat:@"%@&latitude=%@&longitude=%@", url, latitude, longitude]];
    //NSURLRequest *request = [NSURLRequest requestWithURL:url cachePolicy:NSURLRequestReloadIgnoringLocalCacheData timeoutInterval:10.0];
    NSMutableURLRequest * request = [NSMutableURLRequest requestWithURL:completeURL cachePolicy:NSURLRequestReloadIgnoringCacheData timeoutInterval:10.0];
